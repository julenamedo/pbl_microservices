...
# Blocked IP-s
async def register_blocked_ip(ip_address: str, reason: str = "unspecified"):
    """Registra una IP bloqueada en InfluxDB."""
    try:
        point = Point("blocked_ips") \
            .field("ip", ip_address) \
            .field("reason", reason) \
            .time(datetime.utcnow().isoformat())
        write_api.write(bucket=INFLUXDB_BUCKET, org=INFLUXDB_ORG, record=point)
        logger.info(f"IP {ip_address} registrada como bloqueada en InfluxDB.")
    except Exception as e:
        logger.error(f"Error al registrar IP en InfluxDB: {e}")
        raise Exception(f"Error al registrar IP en InfluxDB: {e}")


async def get_blocked_ips(limit: int = 100):
    """Obtiene la lista de IPs bloqueadas desde InfluxDB."""
    try:
        query_api = influxdb_client.query_api()
        query = f"""
            from(bucket: "{INFLUXDB_BUCKET}")
              |> range(start: -30d)  # Ajusta el rango según lo necesites
        ...
        logger.error(f"Error al obtener IPs bloqueadas de InfluxDB: {e}")
        raise Exception(f"Error al obtener IPs bloqueadas de InfluxDB: {e}")
