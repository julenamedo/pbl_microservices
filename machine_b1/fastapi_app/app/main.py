...
async def startup_event():
    try:
        ...
        data = {
            "message": "INFO - Servicio Machine B1 inicializado correctamente"
        }
        message_body = json.dumps(data)
        routing_key = "machine_b1.startup.info"
        await rabbitmq_publish_logs.publish_log(message_body, routing_key)
    except:
        data = {
            "message": "ERROR - Error al inicializar servicio Machine B1"
        }
        message_body = json.dumps(data)
        routing_key = "machine_b1.startup.error"
        await rabbitmq_publish_logs.publish_log(message_body, routing_key)

@app.on_event("shutdown")
async def shutdown_event():
    data = {
        "message": "INFO - Servicio Machine B1 desregistrado de Consul"
    }
    message_body = json.dumps(data)
    routing_key = "machine_b1.shutdown.info"
    await rabbitmq_publish_logs.publish_log(message_body, routing_key)
